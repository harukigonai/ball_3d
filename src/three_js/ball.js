...
import ball_hit_mp3 from './sounds/ball_hit.mp3'
...
export class Ball {
    ...
    constructor({ uuid, position, vel, gameClient, scene, listener }) {
        this.uuid = uuid
        ...
        this.ball_hit_sound = new THREE.PositionalAudio(this.listener)
        const audioLoader = new THREE.AudioLoader()
        audioLoader.load(ball_hit_mp3, (buffer) => {
            this.ball_hit_sound.setBuffer(buffer)
            this.ball_hit_sound.setRefDistance(5)
        })
        this.mesh.add(this.ball_hit_sound)
    }
    ...
    collideSurface(normal, reposition, surfaceVel) {
        const vel_perp = this.vel
            .clone()
        ...
        if (Math.abs(vel_perp.length()) > 1) {
            this.ball_hit_sound.stop()
    ...
    collidePlayer(player) {
        ...
        const playerCollisionFactor = 1.5
        ...
        if (
            ...
            this.collideSurface(
                ...
                player.vel
                    .clone()
                    .projectOnVector(sideNormal)
                    .multiplyScalar(playerCollisionFactor)
            )
            ...
            if (ballIsWithinHighRange) {
                this.collideSurface(
                    ...
                    player.vel
                        .clone()
                        .projectOnVector(new THREE.Vector3(0, 1, 0))
                        .multiplyScalar(playerCollisionFactor)
                )
                hitPlayer = true
